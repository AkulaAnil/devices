...
export class DisplayBoardComponent implements OnInit {
  ...
  getLevelsInfo(action?: boolean) {
    ...
    this._deviceservice.getlevelsDeptSeviceInfo().subscribe((response: Floor[]) => {
      console.log("getlevelsDeptSeviceInfo_Response=>",response)
      if (response) {
        this.levelInfo = response;
        console.log("levelInfo=>",this.levelInfo)
        this.buildingGroupData = _.groupBy(this.levelInfo,'buildId')
        console.log("this.buildingGroupData=>",this.buildingGroupData)
        if (!action)
